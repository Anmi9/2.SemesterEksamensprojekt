Under Elaboration 3 havde vi fokuseret på at skrive et skelet, der kunne kompilere hurtigtst muligt, selvom vi godt vidste, at vi manglede at forholde os kritisk til vores arkitektur. Den prioritering efterlod et rodet afhængighedsforhold mellem vores lag. For at skabe orden i det orienterede vi os efter _kernel-shell_ tankegangen, som følger Robert C. Martins' _Dependency Rule_:
...
Fordi arkitekturen i vores program var lagdelt og ikke fulgte Martins _Clean Architecture_, valgte vi at tilpasse principperne bag kernel-shell-ideen ved at definere vores applikations- (services) og domænelag som kernen og WPF-laget som shell. Efterfølgende fjernede vi afhængigheder, der gik fra vores services ud til WPF-klasserne. Eksempelvis `TryBookOptimalVehicleAsync()` i `BookingService`-klassen fik ændret sin parameter fra at modtage et ViewModel-objekt til primitive datatyper. Som vi citerer #cite(<martin2012>, form: "prose") for, så sikrede vi, at et (klasse)navn deklareret i shell ikke bliver brugt i kernel.
...
En anden stor arkitektonisk ændring kom, fordi vi lærte, at _Model View View-Model (MVVM)_ arkitekturen, som vi brugte i forbindelse med WPF, var implementeret forkert. Vi havde været forvirret over, hvilken del af mønsteret der ligger i hhv. præsentations- og applikationslaget. Vi havde fejlagtigt placeret alt interaktionslogik i view `CreateBookingView.xaml.cs`. Derfor gik vi igang med at migrere alt dette over til view-model `CreateBookingViewModel.cs`.
